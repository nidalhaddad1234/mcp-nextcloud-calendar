/**
 * Web-based CalDAV XML Builder for calendar-specific XML operations
...
export class WebCalDavXmlBuilder {
  ...
  buildPropfindRequest(properties?: string[]): string {
    ...
    const doc = this.xmlService.createDocument('d:propfind', WebCalDavXmlBuilder.NAMESPACES);
  ...
  buildMkcalendarRequest(displayName: string, color?: string): string {
    const doc = this.xmlService.createDocument('c:mkcalendar', WebCalDavXmlBuilder.NAMESPACES);
  ...
  buildProppatchRequest(properties: CalendarProperties): string {
    const doc = this.xmlService.createDocument('d:propertyupdate', {
      ...WebCalDavXmlBuilder.NAMESPACES,
  ...
  buildCalendarQueryReport(
    ...
    const doc = this.xmlService.createDocument('c:calendar-query', WebCalDavXmlBuilder.NAMESPACES);
  ...
  buildExpandRecurringEventsRequest(eventUrls: string[], start: Date, end: Date): string {
    const doc = this.xmlService.createDocument('c:calendar-multiget', WebCalDavXmlBuilder.NAMESPACES);
   ...
   */
  private addComponentFilter(doc: WebXmlDocumentBuilder, compFilter: ComponentFilter): void {
   ...
   */
  private addPropertyFilter(doc: WebXmlDocumentBuilder, propFilter: PropertyFilter): void {
